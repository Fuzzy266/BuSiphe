...
namespace Infrastructure.Services
{
    public class OrderService : IOrderService
    {
        ...
        public async Task<Order> CreateOrderAsync(string buyersEmail, int deliveryMethodId, string basketId, Address shippingAddress)
        {
            ...
            //get items from the productRepo
        ...
        public async Task<Order> GetOrderByIdAsync(int id, string buyersEmail)
        {
            var spec = new OrdersSpecsification(id, buyersEmail);
            return await _unitOfWork.Repository<Order>().GetEntityWithSpec(spec);
        }

        public async Task<IReadOnlyList<Order>> GetOrdersForUserAsync(string buyersEmail)
        {
            var spec = new OrdersSpecsification(buyersEmail);
